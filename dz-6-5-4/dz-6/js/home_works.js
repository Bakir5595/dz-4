...
xhr.addEventListener('load', () => {
    ...
    response.forEach((item) => {
        const cardDeadpul = document.createElement('div');
        cardDeadpul.className = 'card_deadpul';
        cardDeadpul.innerHTML = `
            <span>${item.name}</span>
            <p>${item.description}</p>
            <img src="${item.person_photo}" alt="${item.name}">
        `;
        persons.append(cardDeadpul);
    });
});

let newXhr = new XMLHttpRequest();
newXhr.open("GET", '../data/persons.json');
newXhr.send();
